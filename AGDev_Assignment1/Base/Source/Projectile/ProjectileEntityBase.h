...
class CProjectileEntityBase : public EntityBase, public CCollider
{
public:
	CProjectileEntityBase(void);
	CProjectileEntityBase(Mesh* _modelMesh);
	~CProjectileEntityBase(void);
public:
	...
	void SetSource(EntityBase* _source);
	// Get the source of the projectile
	EntityBase* GetSource(void) const;
...
protected:
	...
	EntityBase* theSource;
};

namespace Create
{
	CProjectileEntityBase* ProjectileEntityBase(const std::string& _meshName, 
