...
void Application::Init()
{
	...
	CLuaInterface::GetInstance()->saveIntValue("Player2", 100, "Image/DM2240_HighScore.lua");
...
void Application::Run()
{
	...
	while (!glfwWindowShouldClose(m_window) && !IsKeyPressed(VK_ESCAPE))
	{
		if (m_window_height != CLuaInterface::GetInstance()->getIntValue("height", "Image/Settings.lua"))
		{
			...
			//lfwSetWindowSizeCallback(m_window, resize_callback);
			//GraphicsManager::GetInstance()->Init();
		}
		glfwPollEvents();
		UpdateInput();
